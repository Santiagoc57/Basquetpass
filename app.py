...
MAX_FILE_SIZE_MB = max(50, MAX_FILE_SIZE_MB)  # keep a sane lower bound
...
async def process_endpoint(
    ...
    basket_max_shots: Optional[str] = Form(None),
    basket_pre_seconds: Optional[str] = Form(None),
    basket_post_seconds: Optional[str] = Form(None),
    basket_min_gap_seconds: Optional[str] = Form(None),
):
    basketball_mode = os.environ.get("BASKETBALL_MODE", "").strip().lower() in {"1", "true", "yes", "on"}
    api_key = request.headers.get("X-Gemini-Key")
    if not api_key and not basketball_mode:
    ...
    if "application/json" in content_type:
        ...
        url = body.get("url")
        basket_max_shots = body.get("basket_max_shots", body.get("basketMaxShots", basket_max_shots))
        basket_pre_seconds = body.get("basket_pre_seconds", body.get("basketPreSeconds", basket_pre_seconds))
        basket_post_seconds = body.get("basket_post_seconds", body.get("basketPostSeconds", basket_post_seconds))
        basket_min_gap_seconds = body.get("basket_min_gap_seconds", body.get("basketMinGapSeconds", basket_min_gap_seconds))
    ...
    def parse_optional_int(value: Optional[str], field_name: str) -> Optional[int]:
        ...
        try:
            ...
            raise HTTPException(status_code=422, detail=f"Invalid {field_name}: must be a positive integer")
    ...
    def parse_optional_float(value: Optional[str], field_name: str) -> Optional[float]:
        ...
        try:
            ...
            raise HTTPException(status_code=422, detail=f"Invalid {field_name}: must be a positive number")

    basket_max_shots_val = parse_optional_int(basket_max_shots, "basket_max_shots")
    basket_pre_seconds_val = parse_optional_float(basket_pre_seconds, "basket_pre_seconds")
    basket_post_seconds_val = parse_optional_float(basket_post_seconds, "basket_post_seconds")
    basket_min_gap_seconds_val = parse_optional_float(basket_min_gap_seconds, "basket_min_gap_seconds")
    ...
    env = os.environ.copy()
    if api_key:
        env["GEMINI_API_KEY"] = api_key # Override with key from request when provided
    ...
    if basket_max_shots_val is not None:
        cmd.extend(["--basket-max-shots", str(basket_max_shots_val)])
    if basket_pre_seconds_val is not None:
        cmd.extend(["--basket-pre-seconds", str(basket_pre_seconds_val)])
    if basket_post_seconds_val is not None:
        cmd.extend(["--basket-post-seconds", str(basket_post_seconds_val)])
    if basket_min_gap_seconds_val is not None:
        cmd.extend(["--basket-min-gap-seconds", str(basket_min_gap_seconds_val)])
